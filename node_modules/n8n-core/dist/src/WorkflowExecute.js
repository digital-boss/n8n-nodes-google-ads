...
class WorkflowExecute {
        ...
        return new PCancelable((resolve, reject, onCancel) => {
            ...
            const returnPromise = (async () => {
                ...
                executionLoop: while (this.runExecutionData.executionData.nodeExecutionStack.length !== 0) {
                    ...
                    n8n_workflow_1.LoggerProxy.debug(`Start processing node "${executionNode.name}"`, { node: executionNode.name, workflowId: workflow.id });
                    ...
                    for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
                        ...
                        try {
                            ...
                            n8n_workflow_1.LoggerProxy.debug(`Running node "${executionNode.name}" started`, { node: executionNode.name, workflowId: workflow.id });
                            ...
                            n8n_workflow_1.LoggerProxy.debug(`Running node "${executionNode.name}" finished successfully`, { node: executionNode.name, workflowId: workflow.id });
                            ...
                            this.runExecutionData.resultData.lastNodeExecuted = executionData.node.name;
                            executionError = Object.assign(Object.assign({}, error), { message: error.message, stack: error.stack });
                            n8n_workflow_1.LoggerProxy.debug(`Running node "${executionNode.name}" finished with error`, { node: executionNode.name, workflowId: workflow.id });
                        }
                    }
                    if (!this.runExecutionData.resultData.runData.hasOwnProperty(executionNode.name)) {
    ...
    async processSuccessExecution(startedAt, workflow, executionError) {
        const fullRunData = this.getFullRunData(startedAt);
        if (executionError !== undefined) {
            n8n_workflow_1.LoggerProxy.verbose(`Workflow execution finished with error`, { error: executionError, workflowId: workflow.id });
            ...
            n8n_workflow_1.LoggerProxy.verbose(`Workflow execution will wait until ${this.runExecutionData.waitTill}`, { workflowId: workflow.id });
